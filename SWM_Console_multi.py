...
def get_pet(haude_factor, temperature, humidity, date, parameter_safe):
    ...
    print(time.strftime("%H:%M:%S: ") + "PET berechnet.")
...
def get_aet(pet_raster, water_raster, s_pre_raster, rp_raster, rpwp_dif_raster, wp_raster, date, parameter_safe):
    ...
    print(time.strftime("%H:%M:%S: ") + "AET berechnet.")
...
def get_precipitation(dataspace, date, idw_pow, rastercellsize, parameter_safe):
    ...
    print(time.strftime("%H:%M:%S: ") + "Niederschlag interpoliert.")
...
def get_runoff(water_raster, lambda_param, wp_raster, p_raster, s_pre_raster, fc_raster, pet_raster, date,
    ...
    print(time.strftime("%H:%M:%S: ") + "Gesamtabfluss je Rasterzelle berechnet.")
...
def get_soilwater(s_pre_raster, p_raster, aet_raster, runoff_raster, date, parameter_safe):
    ...
    print(time.strftime("%H:%M:%S: ") + "Bodenwasserspeicher berechnet.")
...
id_yesterday = start = 20030101  # start datum: YYYYMMDD
end = 20030131  # finish datum: YYYYMMDD
s_init = ExtractByMask(Raster(r'C:\HiWi_Hydro-GIS\MTP_HydroGIS_Basisdaten.gdb\FK_von_L'), basin)

# from minimum to maximum by step
rp_factor_min = 0.85
rp_factor_max = 0.85
rp_factor_step = 0.05
c_min = 150
